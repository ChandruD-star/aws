...
resource "aws_dynamodb_table" "movie_table" {
  name           = "${var.dynamodb_table}-${random_string.random.id}"
...
resource "aws_s3_bucket" "lambda_bucket" {
  bucket_prefix = "${var.s3_bucket_prefix}-${random_string.random.id}"
  force_destroy = true
}
...
resource "aws_lambda_function" "apigw_lambda_ddb" {
  ...
  environment {
    variables = {
      DDB_TABLE = "${var.dynamodb_table}-${random_string.random.id}"
    }
  }
...
resource "aws_iam_role" "lambda_exec" {
  name = "LambdaDdbPost-${random_string.random.id}"
...
resource "aws_iam_policy" "lambda_exec_role" {
  name = "lambda-tf-pattern-ddb-post-${random_string.random.id}"
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:dynamodb:*:*:table/${var.dynamodb_table}-${random_string.random.id}"
        },
        {
            "Effect": "Allow",
            "Action": [
